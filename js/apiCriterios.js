...
export const updateResultadoDesejado = async (idResultadoDesejado, bodyCriterio) => {
  const url = `https://usinagem-senai-api.cyclic.app/v1/projeto-usinagem/resultado-desejado/${idResultadoDesejado}`;
  const options = {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
...
export const updateMargemErro = async (idMargemErro, bodyCriterio) => {
  const url = `https://usinagem-senai-api.cyclic.app/v1/projeto-usinagem/margem-erro/${idMargemErro}`;
  const options = {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
...
export const deleteResultadoDesejado = async (idResultado) => {
  const url = `https://usinagem-senai-api.cyclic.app/v1/projeto-usinagem/resultado-desejado/${idResultado}`;
  const options = {
    method: 'DELETE'
  };
  ...
  .then(response => {
    if (response.ok) {
      ...
      console.log('Erro ao deletar o resultado desejado.');
    }
  })
  .catch(error => {
    console.log('Ocorreu um erro na requisição:', error);
...
export const deleteMargemErro = async (idMargemErro) => {
  const url = `https://usinagem-senai-api.cyclic.app/v1/projeto-usinagem/margem-erro/${idMargemErro}`;
  const options = {
    method: 'DELETE'
  };
  ...
  .then(response => {
    if (response.ok) {
      ...
      console.log('Erro ao deletar a margem de erro.');
    }
  })
  .catch(error => {
    console.log('Ocorreu um erro na requisição:', error);
